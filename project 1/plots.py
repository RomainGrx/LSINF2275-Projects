...
@date : 2021 Apr 03, 10:21:29
@last modified : 2021 Apr 05, 11:35:08
"""
...
def plotfun_theoretical_vs_empirical(layout, boolean, number_of_sim, strategy):
    ...
    plt.legend(
        ("100 Simulations", "10000 Simulations", "1000000 Simulations", "Theoretical"),
        fontsize=16,
    )
    plt.show
...
def plotfun_strategies_comparison(layout, boolean, num_of_sim, strategies):
    ...
    plt.yticks(fontsize=14)
    plt.legend(
        (strategies[0], strategies[1], strategies[2], strategies[3]), fontsize=16
    )
    plt.show
...
def plotfun_relative_error(layout, boolean, number_of_sim, strategy):
    ...
    plt.plot(
        Square_nb,
        np.divide(np.subtract(theor, emp[0]), theor),
        "#e41a1c",
        linewidth=0.7,
        marker="o",
    )
    plt.plot(
        Square_nb,
        np.divide(np.subtract(theor, emp[1]), theor),
        "#377eb8",
        linewidth=0.7,
        marker="o",
    )
    plt.plot(
        Square_nb,
        np.divide(np.subtract(theor, emp[2]), theor),
        "#4daf4a",
        linewidth=0.7,
        marker="o",
    )
    plt.plot(
        Square_nb,
        np.divide(np.subtract(theor, emp[3]), theor),
        "#984ea3",
        linewidth=0.7,
        marker="o",
    )
    plt.plot(
        Square_nb,
        np.divide(np.subtract(theor, emp[4]), theor),
        "#ff7f00",
        linewidth=0.7,
        marker="o",
    )
    ...
    plt.yticks(fontsize=14)
    plt.legend(
        (strategy[0], strategy[1], strategy[2], strategy[3], strategy[4]), fontsize=16
    )
    plt.show
...
plt.plot(
    Square_nb,
    np.divide(np.subtract(theor, emp1), theor),
    "#e41a1c",
    linewidth=0.7,
    marker="o",
)
plt.plot(
    Square_nb,
    np.divide(np.subtract(theor, emp2), theor),
    "#377eb8",
    linewidth=0.7,
    marker="o",
)
plt.plot(
    Square_nb,
    np.divide(np.subtract(theor, emp3), theor),
    "#4daf4a",
    linewidth=0.7,
    marker="o",
)
...
plt.yticks(fontsize=14)
plt.legend(
    ("Optimal", "Security Die only", "Normal Die only", "Riksy Die only"), fontsize=16
)
plt.show()
